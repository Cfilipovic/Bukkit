...
public class BanCommand extends SubCommand {
    ...
    public BanCommand() {
        super("Ban a player", "/admin ban [player] [length] [mins/hours/days] [reason]", "ban");
    }

    @Override
    public void onCommand(Player p, String[] args) {
        //make sure player has permission
        if(!p.hasPermission("Admin.Ban")) {
            MessageManager.getInstance().msg(p, MessageManager.MessageType.BAD, "You don't have permisssion to ban players.");
            return;
        }
        ...
        Ban ban = new Ban(bannedPlayer.getUniqueId(), moderator.getName(), getTimeStamp(timeType, length), reason);
        ban.ban();
        ban.successMessage(ban);
    }
